...
#define _DEBUG 1
    ...
    // Could use #ifdef _NTDEF_ instead
        ...
        EXT_C void OutputDebugOutputf(const TCHAR * fmt, ...);
    #endif
...
EXT_C void __stdcall SetThreadName( unsigned long dwThreadID, const char * szThreadName);
/// \note This function ASSUMES static const buffers. So don't use auto variables. There's no problem with SetThreadName
EXT_C void __stdcall SetThreadNameOneTime( unsigned long dwThreadID, const char * szThreadName);
#endif
